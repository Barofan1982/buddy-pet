...
import hashlib, json, math, random, sys, time, tkinter as tk
...
def do_tick(data: dict) -> dict:
    now     = time.time()
    elapsed = max(0.0, min(now - data.get('last_tick', now), 8 * 3600))  # 防时钟回拨，离线最多补算8h
    data['last_tick'] = now
...
_D = {
    ...
    'hungry': {
        ...
        'axolotl':  ["好奇心……被饿没了……", "……（不想探索了，只想吃）……", "呜……肚子在叫我回来吃东西……"],
        'rabbit':   ["啊！我要饿死了！！（过激反应）", "主人！！！！我好饿！！！！", "……（眼泪要掉下来了）……好饿……"],
        'turtle':   ["……慢慢地……饿着……", "……没关系……我等得住……（其实很饿）", "……壳都轻了……"],
        'octopus':  ["八条腿……都没力气了……", "……墨水也挤不出来了……饿……", "……（蔫蔫地趴着）……喂我……"],
    ...
    def _refresh(self):
        ...
        if self._aff_lbl:
            self._aff_lbl.config(
                text=f'关系：{aff_label(aff)}  （好感度 {aff}）')
